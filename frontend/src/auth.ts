...
function hideSignup() {
    ...
    overlay.classList.remove("hidden");
    // If we're on the signup URL, navigate back to home
...
form.addEventListener("submit", async (e) => {
    ...
    if (data.twofaRequired) {
        ...
        const retry = await fetch(`${API_BASE_URL}/login`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
// Handle URL-based routing for signup/login
function handleAuthRouting() {
    const path = window.location.pathname;
    if (path === '/signup') {
        // Wait a bit for the DOM to be ready, then show signup
        setTimeout(() => {
            showSignup();
            // Keep the URL as /signup while modal is open
