/**
 * Dynamically fills the History table from an array.
 * Replace `demoData` later with data fetched from your backend API.
 */
...
export function initHistoryTab() {
  if (loaded) return;           // run once
  loaded = true;

  const tbody = document.getElementById("history-body")!;
  const demoData: MatchRow[] = [
